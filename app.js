...
app.post('/withdraw/:aNumber/:id', (req, res) => {
  ...
  con.query('select * from account where id=?', [req.params.id], (err, result) => {
    ...
    const name = result[0].NAME;
    if (apw == result[0].aPW) {
      con.query('insert into transaction(aNUMBER, sendName, MONEY, PLUS, sendNumber, sendbank, NAME) values(?,?,?,?,?,?,?)', [req.params.aNumber, sendName, money, 1, sendNumber, sendbank, name], (err) => {
        con.query('select * from transaction where aNumber = ?',[req.params.aNumber],(err,data)=>{
        if (err) throw err;
